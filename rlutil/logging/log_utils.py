...
def setup_logger(parameters=None, dirname=None, exp_prefix='exp', log_base_dir=LOG_BASE_DIR):
    reset_logger()
    exp_uuid = datetime.datetime.now().strftime("%Y%m%d_%H%M%S_%f")
    if dirname is None:
